...
void delete_trie(struct trie *trie) {
    ...
    if (trie->children)
        delete_trie(trie->children);
    if (trie->sibling)
        delete_trie(trie->sibling);
...
static struct output_list *new_output_link(int label,
...
static void compute_failure_link_for_node(struct trie *v, struct trie *root,
                                          struct queue *queue) {
