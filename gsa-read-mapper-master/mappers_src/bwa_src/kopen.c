...
static int http_open(const char *fn)
{
	...
	buf = calloc(bufsz, 1); // FIXME: I am lazy... But in principle, 64KB should be large enough. id:40 gh:60 ic:gh
...
static int kftp_get_response(ftpaux_t *aux)
{
	...
	while (read(aux->ctrl_fd, &c, 1)) { // FIXME: this is *VERY BAD* for unbuffered I/O id:46 gh:66 ic:gh
	...
	if (strstr(fn, "http://") == fn) {
		...
		if (*p == '<') { // pipe open
			...
			if (pid == 0) { /* the child process */
				char **argv; /* FIXME: I do not know if this will lead to a memory leak id:51 gh:71 ic:gh*/
				close(pfd[0]);
