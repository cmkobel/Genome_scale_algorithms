...
void bsw2_extend_left(const bsw2opt_t *opt, bwtsw2_t *b, uint8_t *_query, int lq, uint8_t *pac, bwtint_t l_pac, uint8_t *_mem)
{
	...
	for (i = 0; i < b->n; ++i) {
		...
		if (score) continue;
		if (lt > p->k) lt = p->k;
		for (k = p->k - 1, j = 0; k > 0 && j < lt; --k) // FIXME: k=0 not considered! id:44 gh:64 ic:gh
...
{
	// FIXME: this routine does not work if the query bridge three reference sequences id:49 gh:69 ic:gh
	...
	if (x > refl) { // then fix it
		...
		for (i = j = 0; i < n_cigar; ++i) {
			...
			if (op == 4 || op == 5 || op == 1) { // ins or clipping
				...
				if (x + ln >= refl && nc == 0) {
					// FIXME: not consider a special case where a split right between M and I id:56 gh:76 ic:gh
...
static void bsw2_aln_core(bsw2seq_t *_seq, const bsw2opt_t *_opt, const bntseq_t *bns, uint8_t *pac, const bwt_t *target, int is_pe)
{
	...
	for (x = 0; x < _seq->n; ++x) {
		...
		for (i = k = 0; i < l; ++i) {
			...
			if (c >= 4) { c = (int)(drand48() * 4); ++k; } // FIXME: ambiguous bases are not properly handled id:35 gh:55 ic:gh
			seq[0][i] = c;
			seq[1][l-1-i] = 3 - c;
			rseq[0][l-1-i] = 3 - c;
			rseq[1][i] = c;
